...
export async function GET() {
    try {
        ...
        // 1. Fetch all help-related data concurrently
        const [rolesRes, respRes, flowRes, faqsRes] = await Promise.all([
            supabase.from('help_roles').select('*').eq('is_active', true).order('display_order', { ascending: true }),
        ...
        const flowSteps = (flowRes.data || []).map((r: any) => ({
            ...r,
