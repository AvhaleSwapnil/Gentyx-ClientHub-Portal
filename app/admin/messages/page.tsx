                            ...
                            <CardContent className="p-0 max-h-[550px] overflow-y-auto">
                                {clientsLoading ? (
                                    ...
                                    <div className="divide-y relative">
                                        {clients.map((client) => {
                                            const unread = isUnread(`client-${client.id}`, client.last_message_at, client.last_message_sender_role, "CLIENT");
                                            return (
                                                <button
                                                    key={client.id}
                                                    onClick={() => {
                                                        setSelectedClient(client);
                                                        markAsRead(`client-${client.id}`);
                                                    }}
                                                    className={cn(
                        ...
                        <div className="lg:col-span-2">
                            {selectedClient ? (
                                <FlexibleChat
                                    ...
                                    recipients={[
                                        { role: "CLIENT", label: selectedClient.name, color: "bg-blue-500" },
                                    ]}
                    ...
                    <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
                        <Card className="lg:col-span-1">
                            ...
                            <CardContent className="p-0 max-h-[550px] overflow-y-auto">
                                {scLoading ? (
                                    ...
                                    <div className="divide-y relative">
                                        {filteredSCs.map((sc) => {
                                            const unread = isUnread(`sc-${sc.id}`, sc.last_message_at, sc.last_message_sender_role, "SERVICE_CENTER");
                                            return (
                                                <button
                                                    key={sc.id}
                                                    onClick={() => {
                                                    ...
                                                    className={cn(
                                                        "w-full flex items-center gap-3 p-3 text-left hover:bg-muted/50 transition-colors",
                                                        selectedSC?.id === sc.id && "bg-emerald-50 border-l-4 border-emerald-500"
                                                    )}
                                                >
                                                    <div className={cn(
                                                        "h-12 w-12 rounded-full flex items-center justify-center text-sm font-semibold shrink-0 relative",
                                                    ...
                                                    <div className="flex-1 min-w-0">
                                                        <div className="flex justify-between items-start">
                                                            <p className={cn(
                                                                "font-medium truncate text-sm",
                                                                selectedSC?.id === sc.id && "text-emerald-700",
                                                                unread && "font-bold text-slate-900"
                                                            )}>
                                                                {sc.name}
                                                            </p>
                                                        ...
                                                        <p className={cn(
                                                            ...
                                                            {sc.last_message_body ? (
                                                                ...
                                                                sc.code ? `Code: ${sc.code}` : "No messages yet"
                                                            )}
                                                        </p>
                                                    </div>
                                                </button>
                        ...
                        <div className="lg:col-span-2">
                            {selectedSC ? (
                                <FlexibleChat
                                    clientId="0"
                                    serviceCenterName={selectedSC.name}
                                    serviceCenterId={selectedSC.id}
                                    currentUserRole="ADMIN"
                                    recipients={[
                                        { role: "SERVICE_CENTER", label: selectedSC.name, color: "bg-emerald-500" },
                                    ]}
                                    height="550px"
                                />
                            ) : (
                    ...
                    <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
                                ...
                                {cpaLoading ? (
                                    ...
                                    <div className="divide-y relative">
                                        {filteredCPAs.map((cpa) => {
                                            const unread = isUnread(`cpa-${cpa.id}`, cpa.last_message_at, cpa.last_message_sender_role, "CPA");
                                            return (
                                                <button
                                                    key={cpa.id}
                                                    onClick={() => {
                                                        setSelectedCPA(cpa);
                                                        markAsRead(`cpa-${cpa.id}`);
                                                    }}
                                                    className={cn(
                                                        "w-full flex items-center gap-3 p-3 text-left hover:bg-muted/50 transition-colors",
                                                        selectedCPA?.id === cpa.id && "bg-amber-50 border-l-4 border-amber-500"
                                                    )}
                                                >
                                                    <div className={cn(
                                                        "h-12 w-12 rounded-full flex items-center justify-center text-sm font-semibold shrink-0 relative",
                                                        selectedCPA?.id === cpa.id
                                                    ...
                                                    <div className="flex-1 min-w-0">
                                                        <div className="flex justify-between items-start">
                                                            <p className={cn(
                                                                "font-medium truncate text-sm",
                                                                selectedCPA?.id === cpa.id && "text-amber-700",
                                                                unread && "font-bold text-slate-900"
                                                            )}>
                                                                {cpa.name}
                                                            </p>
                        ...
                        <div className="lg:col-span-2">
                            {selectedCPA ? (
                                <FlexibleChat
                                    clientId="0"
                                    cpaId={selectedCPA.id}
                                    currentUserRole="ADMIN"
                                    recipients={[
                                        { role: "CPA", label: selectedCPA.name, color: "bg-amber-500" },
                                    ]}
                                    height="550px"
                                />
                            ) : (
