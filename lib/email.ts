 ...
 * Includes logging to the database for admin visibility
 */
export async function sendEmail(options: SendEmailOptions | SendEmailWithLoggingOptions) {
  ...
  const sender = process.env.SMTP_FROM || process.env.NEXT_PUBLIC_SMTP_FROM || "no-reply@clienthub.com";
...
export async function getClientEmail(clientId: number | string): Promise<{ email: string; name: string } | null> {
  try {
    ...
    const result = await pool.request()
      .input("clientId", Number(clientId))
      .query(`
