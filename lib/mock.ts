import type { AuditLog, DocumentFile, EmailTemplate, Message, Stage, Task, UserRole } from "@/types"
type ClientProfile = any;
...
export const mockClients: ClientProfile[] = Array.from({ length: 10 }).map((_, i) => {
  ...
  return {
    client_id: i + 1,
    client_name: `Client ${i + 1} LLC`,
    id: `cli-${i + 1}`,
    name: `Client ${i + 1} LLC`,
    primary_contact_name: `Contact ${i + 1}`,
    primary_contact_email: `client${i + 1}@mail.com`,
    primary_contact_phone: "555-0101",
    service_center_id: i % 3 + 1,
    cpa_id: i % 3 + 1,
