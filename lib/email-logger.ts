...
export async function updateEmailLogStatus(
    ...
    try {
        ...
        const updateData: any = {
            ...
            acs_message_id: acsMessageId || undefined,
        };
...
export async function logResendAttempt(
    ...
    try {
        ...
        // Fetch current resend count first
        ...
        const { error } = await supabase
            .from('email_logs')
            .update({
                resend_count: (currentEntry.resend_count || 0) + 1,
...
export async function fetchEmailLogs(filters: EmailLogFilters): Promise<EmailLogsResponse> {
    ...
    // Map to camelCase to maintain legacy compatibility if needed
